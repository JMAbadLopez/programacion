...
!!! question "💻 Reto Final de Unidad: El Zoológico Interactivo 🦁🐬🐶"
    ...
    1.  **Clase Abstracta `Animal`:**
        ...
        *   Método Abstracto **`comer(String comida)`**: Cada animal recupera energía de forma distinta según lo que coma.
    2.  **Interfaz `Trucos`:**
    ...
    3.  **Las Especies:**
        ...
        *   **`Delfin`:** Come "Peces" (+50) y "Calamar" (+80). Implementa `Trucos`: *"¡Hace un salto mortal hacia atrás!"*.
        *   **`Perro`:** Come de todo ("Carne" +50, "Pienso" +30). Implementa `Trucos`: *"¡Da la patita y rueda!"*.
    4.  **El Show (Main):**
