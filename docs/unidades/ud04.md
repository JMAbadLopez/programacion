...
En el desarrollo de software profesional, la gestión de datos rara vez es predecible. Hasta ahora, hemos utilizado **Arrays** (arreglos), que son estructuras de almacenamiento **estático**. Esto implica que su tamaño se define en tiempo de compilación o inicialización y no puede modificarse durante la ejecución.
...
```mermaid
graph LR
    subgraph "Claves (Keys) - Únicas"
    K1[1111A]
    K2[2222B]
    K3[3333C]
    end

    subgraph "Valores (Values) - Pueden repetirse"
    V1[Ana López]
    V2[Carlos Ruiz]
    V3[Beatriz Aranda]
    end
