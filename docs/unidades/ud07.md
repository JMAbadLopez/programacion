...
## 7. Ciclo de Vida Completo: Guardando y Cargando Objetos
...
### 7.1. Serialización Manual de Texto
...
public class Personaje {
    ...
    public void mostrarInfo() {
        // Usamos Locale.US para que la salida por consola también use punto decimal
        System.out.printf(Locale.US, "Héroe: %s | Nivel: %d | Salud: %.1f\n", nombre, nivel, puntosVida);
    }
    ...
    public void guardarPartida(File archivoDestino) throws FileNotFoundException {
        ...
        try (PrintStream escritor = new PrintStream(archivoDestino)) {
            // Forzamos el uso del punto como separador decimal (Estándar US)
            escritor.useLocale(Locale.US);
    ...
    public static Personaje cargarPartida(File archivoOrigen) throws FileNotFoundException {
        // Usamos try-with-resources para asegurar el cierre del flujo
        try (Scanner lector = new Scanner(archivoOrigen)) {
            // Forzamos al Scanner a buscar puntos decimales, no comas
            lector.useLocale(Locale.US);
            
            String nom = lector.nextLine();
            int niv = lector.nextInt();
            double vida = lector.nextDouble();
            
            return new Personaje(nom, niv, vida);
        }
    }
}
```

### 7.2. El Bucle Principal del Juego (Main)
