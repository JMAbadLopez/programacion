...
System.out.println("Total creados: " + Personaje.getContadorPersonajes()); // Imprime 2
```

### 6.1 Constantes estáticas

El la UD1 estudiamos el uso de constantes, que son aquellas que se identifican con la palabra `final'`.
...
public class Personaje {
    ...
    public void setNivel(int nivel) {
        if (nivel < 0) {
            System.out.println("Error: Nivel negativo no permitido.");
            this.nivel = 0;
        } else {
            this.nivel = nivel;
        }
