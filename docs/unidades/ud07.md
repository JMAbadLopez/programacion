...
* `createNewFile()`: Crea un nuevo archivo vacío físicamente en el disco. Devuelve `true` si lo creó con éxito, o `false` si el archivo ya existía. Requiere control de excepciones (`IOException`).
* `mkdir()`: Crea un único directorio (carpeta) en la ruta especificada.
* `mkdirs()`: Crea un directorio y, de manera recursiva, todos los directorios "padre" intermedios que no existan en la ruta proporcionada.
* `delete()`: Elimina el archivo o directorio. **Nota crítica:** Para que la operación tenga éxito sobre un directorio, este debe estar completamente vacío.
* `renameTo(File dest)`: Cambia el nombre del archivo o lo reubica (mueve) a una nueva ruta especificada por el objeto `File` de destino.
...
!!! question "💻 Momento de Práctica: El Lector de Pergaminos"
    ...
    3.  **El Reto:** ¿Qué pasa si en el archivo cambias el `180,5` por `180.5` (punto en lugar de coma)? Investiga por qué `Scanner` se comporta de forma diferente según el idioma de tu ordenador.
...
!!! question "💻 Momento de Práctica: El Expediente Académico"
    ...
    3.  Modifica el programa para que lea **todas las líneas** del archivo usando un bucle `while(sc.hasNext())`, cree un objeto para cada estudiante y lo guarde en un `ArrayList<Alumno>`.
    4.  Al final, recorre el `ArrayList` e imprime la información de todos los alumnos recuperados del archivo usando el método `toString()`.
