...
!!! abstract "Objetivos de la Unidad"
    - Comprender la arquitectura JDBC y su función como capa de abstracción sobre los SGBDR.
    - Configurar dependencias con Gradle y establecer conexiones con `DriverManager`.
    - Ejecutar consultas SQL (SELECT, INSERT, UPDATE, DELETE) usando `PreparedStatement`.
    - Procesar resultados con `ResultSet` y mapearlos a objetos Java (POJOs).
    - Aplicar el patrón DAO para separar la lógica de acceso a datos de la lógica de negocio.
    - Organizar proyectos Java en paquetes (`model`, `database`, `view`) siguiendo la separación de responsabilidades.
...
INSERT INTO articulos VALUES (1, 'Teclado',  100.00, 3);
INSERT INTO articulos VALUES (2, 'Disco Duro 300GB',  500.00, 5);
INSERT INTO articulos VALUES (3, 'Mouse',  80.00, 3);
INSERT INTO articulos VALUES (4, 'Memoria USB',  140.00, 4);
INSERT INTO articulos VALUES (5, 'Memoria RAM',  290.00, 1);
INSERT INTO articulos VALUES (6, 'Disco Duro Extraible 230 GB',  650.00, 5);
INSERT INTO articulos VALUES (7, 'Memoria USB',  279.00, 1);
INSERT INTO articulos VALUES (8, 'DVD ROM',  450.00, 2);
INSERT INTO articulos VALUES (9, 'CD ROM' , 200.00, 2);
INSERT INTO articulos VALUES (10, 'Tarjeta de red',  180.00, 3);
```
...
| :--- | :--- | :--- |
| **`Connection`** | Representa la conexión física con la base de datos. Es el punto de partida (el "túnel" de comunicación). | Iniciar la comunicación, gestionar transacciones y servir como fábrica para crear objetos `Statement` o `PreparedStatement`. |
| **`Statement`** | Se utiliza para ejecutar sentencias SQL estáticas (sin parámetros fijados en tiempo de ejecución) y obtener sus resultados. | Ejecutar consultas simples y estáticas (ej. `SELECT * FROM tabla`). |
| **`PreparedStatement`** | Una subinterfaz de `Statement` que representa una sentencia SQL precompilada. Permite inyectar parámetros (`?`) de forma dinámica y segura. | Ejecutar consultas dinámicas protegiendo contra la **inyección SQL** (ej. `SELECT * FROM tabla WHERE id = ?`). |
| **`ResultSet`** | Representa el conjunto de resultados (la tabla de datos) generado al ejecutar una orden `SELECT`. | Actúa como un cursor o iterador que nos permite recorrer la tabla virtual resultante fila por fila y extraer sus valores. |
...
1. **Preparación**: Se define el texto de la sentencia SQL asegurada (usualmente con las interrogaciones `?` reservando espacios en un `PreparedStatement`) y se le inyectan los valores reales que el usuario haya escrito en el formulario.
    ...
    try (ResultSet rs = pstmt.executeQuery()) {
        if (rs.next()) {
            System.out.println("READ -> Encontrado ID " + rs.getInt("id_articulo") + 
                               ": " + rs.getString("nombre") + " por " + rs.getDouble("precio") + "€");
        } else {
...
try (PreparedStatement pstmt = con.prepareStatement(sqlUpdate)) {
    ...
    pstmt.setDouble(2, 89.99);  // Nuevo precio
...
public class PruebaCRUD {
    ...
    public static void main(String[] args) {
        ...
        try (Connection con = ConexionBD.conectar()) {
            ...
            System.out.println("Fallo fatal de base de datos interrumpiendo el flujo: " + e.getMessage());
        }
    }
}
```
...
public class ArticuloDAO {
    ...
    public boolean insertar(Articulo art) {
        ...
        try (Connection con = ConexionBD.conectar();
            ...
            pstmt.setDouble(3, art.getPrecio());
...
public class AppTienda {
    public static void main(String[] args) {
        ...
        Articulo nuevo = new Articulo(12, "Webcam HD", 45.99, 3);
