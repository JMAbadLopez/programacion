...
### 4.1. Metáfora: El "Puerto Universal" (USB)

Imagina un puerto USB.

*   Puedes conectar un ratón, una impresora, un ventilador o cargar un móvil.
*   Al ordenador no le importa qué dispositivo es. Solo le importa que tenga el **conector USB**.

En programación, una interfaz es ese conector. Es un **contrato de comportamiento**.
Si tu clase "firma el contrato" (implementa la interfaz), garantiza al mundo que tiene ciertos métodos disponibles, sin importar si es un Coche, un Usuario o una Base de Datos.
...
```java
// 2. Clases dispares firman el contrato
public class Coche extends Vehiculo implements Localizable {
    @Override
    public double getLatitud() { return this.gps.lat; }
    // ...
}
```
...
public class Usuario implements Localizable {
    @Override
    public double getLatitud() { return this.movil.lat; }
    // ...
}
```
...
public class Perro implements Localizable {
    @Override
    public double getLatitud() { return this.collar.lat; }
    // ...
}
```
...
```java
// Variable PADRE (Caja) = Objeto HIJO (Contenido)
Personaje p1 = new Guerrero("Conan");
Personaje p2 = new Mago("Gandalf");

// TÚ VES: Un Personaje genérico.
// JAVA VE: Un Guerrero y un Mago reales.

p1.atacar(); // Java busca el método atacar() en la clase GUERRERO -> Espadazo
p2.atacar(); // Java busca el método atacar() en la clase MAGO -> Fuego
```
...
A veces, tienes un `Personaje` (caja genérica) pero sabes que dentro hay un `Guerrero` y quieres usar su método especial `lanzarHacha()` (que el Mago no tiene).

Intentarlo directamente da error:
```java
p1.lanzarHacha(); // ERROR: La clase Personaje no tiene ese método.
```
...
if (p1 instanceof Guerrero) {
    ...
    g.lanzarHacha(); // ¡AHORA SÍ!
}
```

!!! question "💻 Reto Final de Unidad: El Zoológico Interactivo"
    Vamos a juntar todo lo aprendido (Herencia, Abstractas, Interfaces y Polimorfismo).

    1.  **Jerarquía:**
        *   Clase abstracta `Animal` (`nombre`, `comer()`).
        *   Clases `Leon` (come carne), `Delfin` (come peces) y `Perro` (come pienso).
    2.  **Habilidad Especial:**
        *   Interfaz `Trucos` con método `hacerTruco()`.
        *   SOLO el `Delfin` y el `Perro` la implementan (el León es muy orgulloso para hacer trucos).
    3.  **El Show (Main):**
        *   Crea un `ArrayList<Animal>`. Mete un León, un Delfín y un Perro.
        *   Recorre la lista (bucle polimórfico).
        *   Haz que **TODOS** coman.
        *   Intenta que hagan trucos... **¡PERO OJO!** Tienes que usar `instanceof` para preguntar antes. Si es un León, saluda con respeto; si es uno de los otros, hazle hacer el truco.
...
Para comparar el **contenido**, debemos sobreescribir el método `equals` (y `hashCode`, aunque por ahora nos centraremos en `equals`).

```java
@Override
public boolean equals(Object obj) {
    if (this == obj) return true; // Son el mismo objeto en memoria
    if (obj == null || getClass() != obj.getClass()) return false; // No es el mismo tipo
...
public class Personaje implements Comparable<Personaje> {
    ...
    public int compareTo(Personaje otro) {
        // Ordenamos por VIDA (de menos a más)
        return this.vida - otro.vida;
        
        // Si quisiéramos por nombre alfabéticamente:
        // return this.nombre.compareTo(otro.nombre);
    }
}
```

### 5.4.3. Orden Personalizado (`Comparator`)

¿Y si queremos ordenar por diferentes criterios (unos por vida, otros por nombre)?
No tocamos la clase. Creamos "Comparadores" externos.

```java
import java.util.Comparator;

// Opción A: Clase anónima o Lambda (Java 8+)
Comparator<Personaje> porNombre = (p1, p2) -> p1.getNombre().compareTo(p2.getNombre());

// Opción B: Uso directo en el sort
lista.sort(Comparator.comparing(Personaje::getNombre));
```
...
public abstract class Personaje implements Atacable {
    ...
    // Método abstracto: Cada hijo ataca distinto
    public abstract void atacar(Personaje objetivo);

    // Método común implementado de la interfaz
    @Override
...
public class Guerrero extends Personaje {
    ...
    public void atacar(Personaje objetivo) {
        // El guerrero hace más daño pero gasta furia (si la usáramos)
        int dano = this.nivel * 2 + 10;
        System.out.println(this.nombre + " lanza un ESPADAZO BRUTAL a " + objetivo.getNombre());
        objetivo.recibirDano(dano);
    }

    // Método exclusivo del Guerrero
    public void gritoDeGuerra() {
        System.out.println(this.nombre + ": ¡¡POR ESPARTA!!");
    }
}
```
...
public class JuegoRPG {
    public static void main(String[] args) {
        ...
        Guerrero enemigo = new Guerrero("Orco Feo", 200, 3, 0);
...
### 6.5. Diagrama UML Completo del Sistema
